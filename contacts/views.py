...
def inquiry(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            if has_contacted:
                messages.error(request, 'Ya has solicitado información sobre este auto.')
        ...
        if request.recaptcha_is_valid:
            ...
            send_mail(
                'New car Inquiry',
                'You have a new inquiry for the car' + car_title +'.Please login to your admin paner for more info',
                settings.EMAIL_HOST,
                [admin_email],
                fail_silently=False,
            )
